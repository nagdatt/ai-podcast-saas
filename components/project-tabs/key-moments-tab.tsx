...
export function KeyMomentsTab({ keyMoments }: KeyMomentsTabProps) {
  ...
  return (
    <div className="glass-card rounded-2xl p-8">
      <h3 className="text-2xl font-bold mb-8 gradient-purple-text">
        Key Timestamps
      </h3>
      <div className="space-y-6">
        {keyMoments.map((moment, idx) => (
          <div
            key={`${idx}-${moment.time}`}
            className="flex items-start gap-4 md:gap-6 p-4 md:p-6 glass-card rounded-xl border-l-4 border-l-purple-400"
          >
            <Badge className="mt-1 gradient-purple text-white px-3 py-2 text-sm md:text-base font-bold shadow-md shrink-0">
              {moment.time}
            </Badge>
